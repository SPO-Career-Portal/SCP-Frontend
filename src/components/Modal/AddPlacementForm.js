...
class AddPlacement extends Component {
    ...
    handleSubmit = async(event) => {
        ...
        let batches = []
        this.state.selectedBatch.map((i) => {batches.push(i['name'])})
        let branches = []
        this.state.selectedDepartment.map((i) => {branches.push(i['name'])})
        let programmes = []
        this.state.selectedProgramme.map((i)=> {programmes.push(i['name'])})

        await axios.post(base_url+'/admin/addPlacement/', {
                    placement_name: this.state.data['Name'],
