...
const TableContainer = ({ columns, data, changeData }) => {
  ...
  const handleDownload = async (event, cells) => {
    ...
    await axios
      .get(
        base_url + "/admin/downloadIntern/" + fetchedData[index]["key"] + "/"
      )
      .then((res) => {
        setCsvData(res.data);
  ...
  const handleDeleteConfirm = async (event, index) => {
    ...
    await axios
      .delete(base_url + "/admin/deleteIntern/", {
        headers: {
  ...
  return (
    <Fragment>
      ...
      <Modal isOpen={isAddModal} toggle={handleModal}>
        <ModalBody>
          <Add toggle={handleModal} reload={setChanged} offer={"Intern"} />
        </ModalBody>
      </Modal>
